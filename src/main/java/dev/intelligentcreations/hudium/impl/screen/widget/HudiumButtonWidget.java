...
public final class HudiumButtonWidget extends ButtonWidget {
    ...
    public void drawWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        if (isHovered()) {
            guiGraphics.drawNineSlicedTexture(Constants.WIDGETS_TEXTURE, this.getX(), this.getY(), this.getWidth(), this.getHeight(), 1, 1, 200, 20, 0, 20);
        } else {
            guiGraphics.drawNineSlicedTexture(Constants.WIDGETS_TEXTURE, this.getX(), this.getY(), this.getWidth(), this.getHeight(), 1, 1, 200, 20, 0, 0);
        }
        RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, 1.0f);
        int i = this.active ? 0xFFFFFF : 0xA0A0A0;
        this.drawScrollableText(guiGraphics, minecraftClient.textRenderer, i | MathHelper.ceil(this.alpha * 255.0f) << 24);
    }
}
